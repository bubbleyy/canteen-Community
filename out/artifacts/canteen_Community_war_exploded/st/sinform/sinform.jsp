...
<body><div style="display: flex;justify-content: space-between;width: 100%;align-items: center;">
  ...
  <div style="margin-right: 40px;">
    <a id="hiddenprint3" class="btn btn-primary"  href="${pageContext.request.contextPath}/st/sinform/ainformadd.jsp">添加公告</a>
  </div>
</div>
<form action="${pageContext.request.contextPath}/SInformServlet" method="post"  id="forsearchform">
...
<c:choose>
  <c:when test="${gonggaos.size() == 0}">
    <div style="margin: 80px auto;display: flex;justify-content: center;flex-wrap: wrap">
      ...
      <p style="width: 100%;text-align: center;color: #8a8a8a">还没有任何公告信息喔~</p>
    </div>
  </c:when>
  <c:otherwise>
  ...
  <form id="delselectform" name="excel" action="#" method="post">
    <table border="1" class="table table-bordered table-hover">
      ...
      <c:forEach items="${gonggaos}" var = "gonggao" varStatus="s">
        <tr style="font-size: 14px">
          <td>${gonggao.id}</td>

          <c:choose>
          <c:when test="${ not empty gonggaopictures[s.count-1][0]}">
            <td>
              <img src="${pageContext.request.contextPath}/${gonggaopictures[s.count-1][0]}" style="width: 40px;height: 40px;">
            </td>
          </c:when>
          <c:otherwise>
          ...
          <td>${gonggao.title}</td>
          <td>${gonggao.maintext}</td>
          <td>${gonggao.type}</td>
          <td>${gonggao.fromwho}</td>
          <td>${gonggao.looknumber}</td>
          <td>

            <a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/SInformUpdateServlet?id=${gonggao.id}">修改</a>&nbsp;
            <a class="btn btn-default btn-sm" href="javascript:delst(${gonggao.id})">删除</a>
          </td>
        </tr>
      </c:forEach>
    </table>
...
<script>
  ...
  function delst(id) {
    if (confirm("您确定要删除此食堂公告吗")){
      document.getElementById("zhezhaoceng").style.display = 'flex'
      location.href="${pageContext.request.contextPath}/SInformDeleteServlet?id="+id;
    }
  }

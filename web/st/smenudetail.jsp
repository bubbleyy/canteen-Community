...
<nav class="navbar navbar-expand-lg navbar-dark  fixed-top" style="margin-bottom: 0 !important;">
  <div class="container-fluid" style="background-color: white;border-bottom: 2px solid #bfbfbf;padding: 6px 16px;">
    <a class="navbar-brand" href="${pageContext.request.contextPath}/IndexServlet" style="color: black">${menu.name}</a>
...
<div style="margin: 0 40px;">
  ...
  <section style="margin-top: 40px" >
    <div style="width: 100%;height: 650px;border-radius: 10px;display: flex;flex-direction: row;align-items: center">
      ...
      <div  style="width: 50%;height: 600px;margin: 0 5%;overflow: scroll">
        <p>菜品图片</p>
        <c:forEach items="${menupictures}" var = "item" varStatus="s">
      ...
      <div style="width: 30%;height: 600px;margin: 0 5%;">
        <p>菜品名称 ${menu.name}</p>
        <p>摊位 ${st.name} ${menu.tw}</p>
        <p>价格 ${menu.money}</p>
        <p>类型 ${menu.type}</p>
        <p>促销说明 ${menu.cuxiao}</p>
        <p>浏览量 ${menu.looknumber}</p>
        <p>介绍 ${menu.introduce}</p>
        <c:choose>
          <c:when test="${logintype.equals('师生用户')&& !menuuserpinglun}">

            <div  style="width: 100%">
              <h3>评论菜品</h3>
              <form id="AddInform" action="${pageContext.request.contextPath}/MenuPingLunAddServlet" method="post" enctype="multipart/form-data" >
          ...
          <c:otherwise>
            <c:if  test="${menuuserpinglun && logintype.equals('师生用户')}">
              <p>您已评论过该菜品</p>
            </c:if>
  ...
  <section >
    ...
    <div style="width: 100%;display: flex;flex-wrap: wrap;flex-direction: row;background-color: white">
      <c:choose>
        <c:when test="${usermenupingluns.size() == 0}">没有评论信息喔~</c:when>
        <c:otherwise>
          <c:forEach items="${usermenupingluns}" var="post" varStatus="s" >
            ...
            <div style="width: 100%;display: flex;flex-wrap: wrap;margin: 20px 10px;justify-content: center">
              <div style="width: 100%;display: flex;flex-direction: row;margin: 5px 0 ;">
               ...
               ${post.user.username} ${post.menupinglun.pf}星
              </div>
              <div style="width: 100%;margin:5px 0 ;">评论内容：${post.menupinglun.maintext}</div>
            </div>
            <div style="text-align: right"><a href="${pageContext.request.contextPath}/MenupinglunHuifuServlet?id=${menu.id}">查看食堂回复</a></div>

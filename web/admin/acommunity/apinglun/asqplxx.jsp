...
<body>
<form action="${pageContext.request.contextPath}/AcommunitypinglunsearchServlet" method="post"  id="forsearchform">
  <div style="position: absolute;top: 0;background-color: #007BFF;display: flex;align-items: center;justify-content: space-between;right: 0px">
    ...
    <div style="width: 300px;height: 50px;display: flex;justify-content: center;align-items: center;flex-direction: row;">
      <div style="width: 200px;display: flex;flex-direction: row;align-items: center">
        <input type="text" class="form-control" id="name" name="name" placeholder="社区评论内容搜索">
      </div>
...
<c:choose>
  <c:when test="${communitypingluns.size() == 0}">
    <div style="margin: 80px auto;display: flex;justify-content: center;flex-wrap: wrap">
      ...
      <p style="width: 100%;text-align: center;color: #8a8a8a">还没有任何社区评论信息喔~</p>
    </div>
  </c:when>
  <c:otherwise>
  ...
  <form id="delselectform" name="excel" action="#" method="post">
    <table border="1" class="table table-bordered table-hover">
      ...
      <c:forEach items="${communitypingluns}" var = "communitypinglun" varStatus="s">
        <tr style="font-size: 14px">
          <td>${communitypinglun.id}</td>
          <td>${communitypinglun.community_id}</td>
          <c:choose>
          ...
          <td>${communitypinglun.maintext}</td>
          <td>${communitypinglun.user_username}</td>
          <td>
            <a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/AcommunitydetailServlet?id=${communitypinglun.community_id}">查看原文</a>
            <a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/AcommunitypinglunUpdateServlet?id=${communitypinglun.id}">修改</a>&nbsp;
            <a class="btn btn-default btn-sm" href="javascript:delst(${communitypinglun.id})">删除</a>
          </td>
        </tr>
      </c:forEach>
    </table>
...
<script>
  ...
  function delst(id) {
    if (confirm("您确定要删除此社区评论信息吗")){
      document.getElementById("zhezhaoceng").style.display = 'flex'
      location.href="${pageContext.request.contextPath}/AcommunitypinglundeleteServlet?id="+id;
    }
